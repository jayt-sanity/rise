...
- The app includes a supportive `Track` area for daily habits and self-care checklists, with weekly/monthly summaries and prior-period comparisons. Habit progress represents observed consistency, not a performance score.
- The bottom-nav destination is labeled `Plan` and uses three internal views: Schedule (day chapters), Checklist, and Progress. The schedule is grouped into Morning, Focus, and Evening rather than requiring rigid time blocks.
- The product is branded `RISE` with the subtitle “Train like I’ve never won. Compete like I’ve never lost.” Its primary statement is “My biggest project is me.” Navigation is Today, Focus, Plan, Journal, and Progress; calendar and weekly review live within Progress.
- Today prioritizes a morning/daily intention. The overthinking reset is not shown on the Today homepage.
- Journal includes Entries, searchable Tags, and a personal Quotes collection where the user can add a quote with an optional author or source.
- The prototype is a responsive web app, not a phone-frame-only mobile prototype. Desktop uses a persistent left navigation and wider content grids; mobile uses bottom navigation.
...
- The user explicitly requested replacement of the phone-only runtime with a responsive web-app shell. `src/App.tsx` now mounts the web prototype directly; do not restore the phone frame unless requested.
